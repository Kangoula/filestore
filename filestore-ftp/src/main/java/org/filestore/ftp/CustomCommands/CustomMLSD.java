...
//import com.miage.ftpstore.VirtualFile;
 ...
 */
public class CustomMLSD extends AbstractCommand {

    private final Logger LOG = LoggerFactory.getLogger(CustomMLSD.class);
    ...
    public void execute(final FtpIoSession session,
        ...
        try {
            ...
            try {
                ...
                String listFiles = directoryLister.listFiles(parsedArg, session.getFileSystemView(), formater);
                listFiles = filterFiles(listFiles);
                dataConnection.transferToClient(session.getFtpletSession(), listFiles);
    ...
    private String filterFiles(String filesList){
        /*String[] list = filesList.split("\r\n");

        if(!filesList.isEmpty() && list.length > 0) {
            String filtered =
                    Arrays.stream(list)
                    .map(VirtualFile::fromString)
